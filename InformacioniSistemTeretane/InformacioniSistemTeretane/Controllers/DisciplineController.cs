...
namespace InformacioniSistemTeretane.Controllers
{
    ...
    public class DisciplineController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("TakmicenjeId,Naziv,Opis,MaxUcesnika")] Disciplina d)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                foreach (var error in ModelState)
                {
                    ...
                    {
                        _logger.LogDebug("Greška u polju {Field}: {Error}",
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            if (d == null)
            {
                _logger.LogWarning("Disciplina sa ID-om {DisciplinaId} nije pronađena za uređivanje (korisnik: {UserName})",
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,TakmicenjeId,Naziv,Opis,MaxUcesnika")] Disciplina d)
        {
            ...
            if (id != d.Id)
            {
                _logger.LogWarning("ID u putanji ({PutanjaId}) i ID discipline ({DisciplinaId}) se ne podudaraju (korisnik: {UserName})",
                    id, d.Id, User.Identity.Name);
                return NotFound();
            }
            ...
            try
            {
                _context.Update(d);
                await _context.SaveChangesAsync();
                _logger.LogInformation("Disciplina {Naziv} (ID: {DisciplinaId}) uspješno ažurirana od strane {UserName}",
                    d.Naziv, d.Id, User.Identity.Name);
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    _logger.LogWarning("Disciplina sa ID-om {DisciplinaId} nije pronađena (korisnik: {UserName})",
                        d.Id, User.Identity.Name);
                    return NotFound();
                }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            if (d == null)
            {
                _logger.LogWarning("Disciplina sa ID-om {DisciplinaId} nije pronađena za brisanje (korisnik: {UserName})",
                    id, User.Identity.Name);
                return NotFound();
            }

            _logger.LogInformation("Korisnik {UserName} pokreće brisanje discipline: {DisciplinaNaziv} (ID: {DisciplinaId})",
                User.Identity.Name, d.Naziv, d.Id);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            // Provjera da li disciplina ima takmičare
            if (d.Takmicari != null && d.Takmicari.Any())
            {
                _logger.LogWarning("Pokušaj brisanja discipline {DisciplinaNaziv} koja ima takmičare (korisnik: {UserName})",
                    d.Naziv, User.Identity.Name);
            ...
            try
            {
                ...
                await _context.SaveChangesAsync();
                _logger.LogInformation("Disciplina {Naziv} (ID: {DisciplinaId}) uspješno obrisana od strane {UserName}",
                    d.Naziv, d.Id, User.Identity.Name);
            ...
            {
                _logger.LogError(ex, "Greška pri brisanju discipline ID: {DisciplinaId} (korisnik: {UserName})",
                    id, User.Identity.Name);
